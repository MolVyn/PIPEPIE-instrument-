...
bool EncryptFile(const std::string& inputFile, const std::string& outputFile) {
    ...
    try {
        ...
        if (plaintext.empty()) {
            std::cerr << "✖ File read error or file is empty!\n";
            return false;
        }
...
bool DecryptFile(const std::string& inputFile, const std::string& outputFile,
    ...
    try {
        ...
        if (ciphertext.size() <= AES::BLOCKSIZE) {
            std::cerr << "✖ Not enough data or corrupted file.\n";
            return false;
        }
        ...
        std::cout << "\n✔ Decryption completed: " << outputFile << "\n";
        return true;
...
void AnimateSearching(const std::vector<std::string>& folderNames) {
    for (int i = 0; i < 10; ++i) {
        ...
        std::cout << "\r Searching in: " << folderNames[index] << "..." << std::flush;
...
void FindFilesOrDirs() {
    while (true) {
        ...
        if (option == 1) {
            ...
            while (true) {
                std::cout << "Enter type (f — file, dir — directory): ";
                std::getline(std::cin, type);
                if (type == "f" || type == "dir") break;
                std::cout << "✖ Invalid type!\n";
            }

            std::cout << "Enter the name of the file or folder to search (extension optional): ";
        ...
        if (mainChoice == 1) {
            ...
            do {
                ...
                if (choice == 1) {
                    ...
                    std::cout << "Enter path to encrypted file: ";
                    std::getline(std::cin, input);
                    std::cout << "Enter output path for decrypted file: ";
                    std::getline(std::cin, output);
                    std::cout << "Enter Key1 (HEX): "; std::getline(std::cin, k1);
                    std::cout << "Enter Key2 (HEX): "; std::getline(std::cin, k2);
                    std::cout << "Enter Key3 (HEX): "; std::getline(std::cin, k3);
